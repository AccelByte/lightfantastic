...
namespace Unity.StadiaWrapper
{
    public partial struct GgpGamerStatInfo
    {
        ...
        internal unsafe struct gamer_stat_namespace_fixed
        {
            private unsafe fixed byte gamer_stat_namespace_val[totalsize];
            private const int totalsize = 128;
            ...
            public unsafe byte[] Data
            {
                ...
                set
                {
                    ...
                    for (int i = 0; i < passedSize; ++i)
                    {
                        gamer_stat_namespace_val[i] = value[i];
                    }
                }
            }
            ...
            public unsafe string DataAsString
            {
                get
                {
                    ...
                    for (int i = 0; i < totalsize; ++i)
                    {
                        if (gamer_stat_namespace_val[i] == 0)
                            break;

                        list.Add(gamer_stat_namespace_val[i]);
                    }
                ...
                set
                {
                    ...
                    for (int i = 0; i < passedSize; ++i)
                    {
                        gamer_stat_namespace_val[i] = bytes[i];
                    }

                    gamer_stat_namespace_val[bytes.Length] = 0;
                }
            }
            ...
            public unsafe void GetDataToExistingBuffer(ref byte[] data)
            {
                ...
                for (int i = 0; i < totalsize; ++i)
                {
                    data[i] = gamer_stat_namespace_val[i];
                }
            }
        }

        
        private gamer_stat_namespace_fixed gamer_stat_namespace_data;

        public byte[] gamer_stat_namespace
        {
            get { return gamer_stat_namespace_data.Data; }
            set { gamer_stat_namespace_data.Data = value; }
        }

        public string gamer_stat_namespaceAsString
        {
            get { return gamer_stat_namespace_data.DataAsString; }
            set { gamer_stat_namespace_data.DataAsString = value; }
        }

        public void gamer_stat_namespaceToExistingBuffer(ref byte[] data)
        {
            gamer_stat_namespace_data.GetDataToExistingBuffer(ref data);
        }
